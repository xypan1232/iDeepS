...
# Science, Albert-Ludwigs-University Freiburg by
# Sita Lange, Steffen Heyne and Daniel Maticzka 2011-2013
#
# Version 1.0 created on 27.08.2013
#
#	This script creates a GSPAN file from input sequences that are compatible
# with the NSPDK or EDEN software from Fabrizio Costa. It creates graphs
# from RNA sequences that portray sequence and structure features employing
# RNAshapes for structure prediction.
#
# Version 1.0 was used in the publications for GraphClust in 2012 by Heyne et al
# and for GraphProt in 2013 by Maticzka et al. Latest change was to correct
# the error of stacking base pairs, where the graph was previously cyclic,
# now it is acyclic (which is important for the graph kernel).
#
############################################################################
...
Options:
  ...
  -wins   [INTEGER] e.g. "50,100,200"
          A list of window sizes to use. If none are given (empty string ''),
          then the entire sequence is taken with no windows. Each window > 1
          required!
  -shift  <INTEGER> e.g. 20
          The shift of the window, relative to the window size given in percent.
          So you give which percent of the window size shall be used for the
          shift. Of course the shift is rounded down to the nearest whole
          number.
          Example: 20 % of a window 150 would result in a step size of 30 nt. It
          is a relative parameter, as you can give different window sizes. If
          you do not give this parameter there is a default shift of 1 nt.
  -cue    Crop unpaired ends. If you give this flag, then the unpaired ends of
          each single structure are ignored. E.g. the structure
          ...(((...))).. becomes just (((...)))
  -stack  Adds stacking information to graphs. This adds an additional vertex
          (type P) for each pair of stacked base-pairs and four edges (type p)
          from each of the involved bases to the new vertex.
  -e      <FLOAT> e.g. 5.0
  ...
  -t      <INTEGER> [1-5] e.g. 3 OR "3=0,4=100,5=200"
          ...
          window length "4=100" means that shape level 4 is used from length
          100nt (window length) The first given length has to be 0! Not
          continuous given levels are allowed!
  -M      <INTEGER> e.g. 10
  ...
  -annotate
          ...
          the following TAB-delimited columns: SEQID, START, END,
          NAMESPACE#LABEL. Labels with the same name-space and SEQID form
  ...
  -match-shape
          ...
          RNAshapes (if structure is given in another way this struct will be
          kept), if this shape is not possible within given energy range,
          produce a specific t graph with only one vertex 'X'. By this the
          instance becomes very unsimilar to all other graphs (for knn)
  -vp     enable graph computation with viewpoints:
          svmsgdnspdk will center on those nucleotides that are given via
          capital letters and ignore those given as lowercase letters
  ...
  -group  <INTEGER> e.g. 5
          Combine/group that number of input seqs into 1 gspan file output name
          is then '<INT>.group.gspan.bz2'
  -sge    Use SGE cluster for each sequence separately
  -sge-submit-host
          <STRING>
          host used to submit SGE jobs
  -sge-submit-user
          <STRING>
          user for ssh access to submit host. needed if called from a clustered
          job because with these getlogin() automation fails.
  -sge-script
          <STRING>
          Use this file instead of the default sge-submit-script.
  -sge-logdir
          stdout directory for SGE call
  -sge-errdir
          sdterr directory for SGE call
  ...
  SGE mode
  -sge-submit-host  "biui.informatik.uni-freiburg.de"
...
my $options = GetOptions(
  ...
  "sge"               => \$i_sge,
  "sge-submit-host=s" => \$i_sge_submit_host,
  "sge-submit-user=s" => \$i_sge_submit_user,
  "sge-script=s"      => \$i_sge_script,
  "jobid=s"           => \$i_jobid,
...
( defined $i_sge_submit_host )
  or $i_sge_submit_host = 'biui.informatik.uni-freiburg.de';

# set the appropriate exports for old and new cluster respectively
my $SGE_BIN_PATH;
my $SGE_EXPORTS;

if ( $i_sge_submit_host eq 'biui.informatik.uni-freiburg.de' ) {
...
# GLOBAL VARIABLES
###############################################################################
my $rnashapes_loc = $CONFIG{RNASHAPES};
...
( defined $i_sge_submit_user ) and $CURRUSER = $i_sge_submit_user;
...
if ($i_sge) {
  my $SCRIPTNAME = $0;
  my $CLUSTERSUBMIT;
  if ( defined $i_sge_script ) {
    $CLUSTERSUBMIT = $i_sge_script;
  } else {
    $CLUSTERSUBMIT =
      $FindBin::Bin . "/StructureLibrary/generic_submit_to_cluster.sh";
  }
  ...
  if ($ssh) {
    my $syscall =
        "ssh $CURRUSER\@$i_sge_submit_host "
      . "'cd $CURRDIR; "
      . "$SGE_EXPORTS "
      . "$SGE_BIN_PATH/qsub -t 1-$SGE_jobs -o $i_sge_errDir/ -e $i_sge_errDir/ "
      . "$CLUSTERSUBMIT $CURRDIR $SCRIPTNAME \"$params\" ' ";
    say STDERR "starting SGE job:";
    say STDERR $syscall;
    system($syscall );
  } else {
...
# set up file for abstract annotation output
if ( $i_abstr and defined $i_abstr_out ) {
  open( $out_abstr, '>', $i_abstr_out )
    or die("error: could not open file '$i_abstr_out'");
}
...
while ( my $seq = shift @used_seqs ) {
  ...
  $i_debug and say STDERR "calculating for sequence " . $tmp_header;
  ...
  if (@struct_meta) {
    $graph_header = getGraphHeader(
      $seq_id,              $seq_header, \@WINDOWS,             $i_shift,
      $i_e,                 $i_c,        $i_t,                  $i_u,
      $i_r,                 $i_M,        $i_crop_unpaired_ends, $i_i,
      $i_sample_min_length, $i_q,        $i_T,                  $seq_len,
      1
    );
  } else {
    $graph_header = getGraphHeader(
  ...
  if ( @struct_meta && !$i_no_structure && !$no_structure_override ) {
    ...
    my $seq_shrep =
      [ $tmp_meta->{$struct_meta}, "ENERGY", "0.00", "SHAPE", $struct_meta ];
    $gi =
      convertShapeWindow( [$seq_shrep], $seq, $seq_len, 1, $gi, $out,
      $graph_header, $i_annotate, $i_abstr, $i_crop_unpaired_ends, $i_stacks,
      $seq );
  ...
  foreach my $win_size (@WINDOWS_used) {
    ...
    if ($change_shape_level) {
      for ( my $i = 0 ; $i < @level_lens ; $i++ ) {
        $curr_t = $i + 1
          if ( $level_lens[$i] != -1 && ( $level_lens[$i] <= $win_size ) );
      }
    ...
    $gi = convert_RNAshapes_output(
      $rnashapesoutput_fh, $gi,       $i_M,                  $out,
      $graph_header,       $win_size, $seq_len,              $curr_t,
      $i_annotate,         $i_abstr,  $i_crop_unpaired_ends, $i_stacks,
      $seq
    );

    ## no (match) shape found at all for this seq
    if ( $gi == $gi_old + 1 ) {
      $gi = convertSeqWindow( "X", 1, 1, $gi, $graph_header, $out, $i_annotate,
        $seq );
...
############################################################################
sub call_RNAshapes {
  my ( $seq_fasta, $rnashapes_location, $win_size, $shift, $e, $c, $t, $u, $r,
    $q, $T, $i, $sample_length, $seqLen, $matchShape )
    = @_;
  ...
  ($rnashapes_location)
    or die(
    "INPUT ERROR in $FUNCTION: the RNAshapes location" . " is compulsory!\n" );
...
sub convert_RNAshapes_output {
  ...
  while ( $line = <$rnashapesoutput> ) {
    ...
    if ( $line =~ /^(\d+)\s+(\d+)$/ ) {
      ...
      if ( @{$win_shreps} > 0 ) {
        ...
        if ($i_add_seq_graph_win) {
          ($curr_gi) =
            ...
            $win_start, $curr_gi, $winHead, $graph_file_hdl, $annotate,
            $orig_seq );
        }
      ...
      $winHead = getWindowHeader( $graphHead, $winSize, $win_start, $win_end,
  ...
  foreach my $shrep ( @{$win_shreps_aref} ) {
    ...
    if ($abstr) {
      ( $structGraph_ref, $curr_gi ) =
        getStructPlusAbstractStructGraph( $shrep, $curr_gi, $win_size_real,
        $cue, $stacks );
...
# disabled for now because RNAshapes does crazy substitutions if it encounters N
...
#        "'${capitalized_win_seq}' != '${win_seq}'" );
  ...
  my $vertice_label =
    ( defined $i_vp and ( $seq_vp[$curr_crop_i_left] =~ /[a-z]/ ) ) ? 'V' : 'v';

  # create backbone vertice of first nucleotide
  push(
    @vert,
    join( ' ',
      $vertice_label, $curr_gi, $seq[$curr_crop_i_left], $curr_abs_pos )
  );
...
# case: '.(' or ')('  or BOS'(' there is no open block, create a new one ".(" or ")("
        ...
        if (@c_stem) {
          ...
          if (@open_blks) {
            ...
            if (@c_mls) {
              extend_or_open_nested_ML( $curr_shrep, \@c_mls, \@p_bp, \@up,
                \@stmbrks, \@open_blks, $idx, $curr_gi );
                ...
                && $stmbrks[-1]->[2] == $p_bp[1] )
              {
                  ...
                  && $open_blks[-1]->[-1] == $tmp_up[0] - 1 )
                {
                  ...
                  die "ERROR: the base-pairs to not match the "
            ...
            if (@c_el) {
              if ( $c_el[-1] == $p_bp[0] ) {
                ...
                print STDERR "TEST - closed EL1:\n"
                  . ( test( $curr_shrep->[0], $idx, $curr_gi, \@newel ) )
                  if ($i_debug);
              } else {
          ...
          # case: '(.(x).(' compare adjacent BP to UP to see if they create a ML
              ...
              print STDERR "TEST - closed UP:\n"
                . ( test( $curr_shrep->[0], $idx, $curr_gi, \@newup ) )
                if ($i_debug);
            }
        ...
        if (@c_stem) {
            ...
            && $up[-1]->[1] + 1 == $p_bp[0] )
          {
            ...
            print STDERR "TEST - closed ML1:\n"
              . ( test( $curr_shrep->[0], $idx, $curr_gi, \@newml, 1 ) )
              if ($i_debug);
          } else {
          ...
          if (@c_up) {
            ...
            if ( $c_up[0] - 1 == $c_bp[0] ) {
              ...
              print STDERR "TEST - closed HL:\n"
                . ( test( $curr_shrep->[0], $idx, $curr_gi, \@newhl ) )
                if ($i_debug);
                ...
                && $stmbrks[-1]->[2] == $p_bp[1] )
              {
                  ...
                  && ( $up[-1]->[1] + 1 == $p_bp[0] ) )
                {
                  ...
                  print STDERR "TEST - closed IL:\n"
                    . ( test( $curr_shrep->[0], $idx, $curr_gi, \@newil ) )
                    if ($i_debug);
  ...
  # save location of first (nucleotide) vertice
  my $current_zero = $curr_gi;

  # save nucleotide-wise abstract annotation here
  my @ntwise_abstract_annot;
 ...
 # build basic graph edges, stacks, and vertices for stems in the abstract graph
  ...
  foreach my $sk (@tmp) {
      ...
      if ( defined $bps{"$i:$j"} ) {
        ...
        if ($stacks) {
          if (@p_bp) {
            ...
            push( @stackgraph,          "e $curr_gi $c_bp[0] p" );
            push( @stackgraph,          "e $curr_gi $c_bp[1] p" );
    ...
    foreach my $r (@stem_relation_idxs) {
      ...
      # add stem nucleotide annotation sequence parts
      # annotations may overlap, eg. the closing stems of an internal loop
      # are also annotated as internal. in the sequence breakdown of these
      # annotations, we go for stem. all further annotations will only be
      # added if there is not yet a stem annotation
      $ntwise_abstract_annot[ $r - $current_zero ] = 'S';
    }
    if ($stacks) {
  ...
  foreach my $hl (@hls) {
    ...
    for ( my $r = $hl->[0] ; $r <= $hl->[1] ; $r++ ) {
      ...
      # add hairpin nucleotide annotation sequence parts
      # if there is no previous annotation with higher priority (stem)
      if ( not defined $ntwise_abstract_annot[ $r - $current_zero ] ) {
        $ntwise_abstract_annot[ $r - $current_zero ] = 'H';
      }
    }
    ++$curr_gi;
  }
  ...
  foreach my $bl (@bls) {
    ...
    for ( my $r = $bl->[0] ; $r <= $bl->[2] ; $r++ ) {
      ...
      # add bulge nucleotide annotation sequence parts
      # if there is no previous annotation with higher priority (stem)
      if ( not defined $ntwise_abstract_annot[ $r - $current_zero ] ) {
        $ntwise_abstract_annot[ $r - $current_zero ] = 'B';
      }
    }
  ...
  foreach my $il (@ils) {
    ...
    for ( my $r = $il->[0] ; $r <= $il->[2] ; $r++ ) {
      ...
      # add internal loop nucleotide annotation sequence parts
      # if there is no previous annotation with higher priority (stem)
      if ( not defined $ntwise_abstract_annot[ $r - $current_zero ] ) {
        $ntwise_abstract_annot[ $r - $current_zero ] = 'I';
      }
    }
  ...
  foreach my $el (@els) {
    ...
    unless ( $el->[0] == $el->[1] ) {
      if ( defined( $stems{ $bps{"$el->[0]:$el->[1]"} } ) ) {
        push( @abstrgraph,
          "e " . $stems{ $bps{"$el->[0]:$el->[1]"} } . " $curr_gi n" );
      } else {
    ...
    unless ( $el->[2] == $el->[3] ) {
      if ( defined( $stems{ $bps{"$el->[2]:$el->[3]"} } ) ) {
        push( @abstrgraph,
          "e $curr_gi " . $stems{ $bps{"$el->[2]:$el->[3]"} } . " n" );
      } else {
    ...
    for ( my $r = $el->[1] ; $r <= $el->[2] ; $r++ ) {
      ...
      # add external loop nucleotide annotation sequence parts
      # if there is no previous annotation with higher priority (stem)
      if ( not defined $ntwise_abstract_annot[ $r - $current_zero ] ) {
        $ntwise_abstract_annot[ $r - $current_zero ] = 'E';
      }
    }
  ...
  foreach my $ml (@mls) {
    ...
    foreach my $bp ( @{$ml} ) {
      if ( defined( $stems{ $bps{"$bp->[0]:$bp->[1]"} } ) ) {
        push( @abstrgraph,
          "e $curr_gi " . $stems{ $bps{"$bp->[0]:$bp->[1]"} } . " n" );
      } else {
  ...
  # the second BP
    ...
    for ( my $r = $pbp_aref->[0] ; $r <= $cbp_aref->[0] ; $r++ ) {
      ...
      # add multiloop nucleotide annotation sequence parts
      # if there is no previous annotation with higher priority (stem)
      if ( not defined $ntwise_abstract_annot[ $r - $current_zero ] ) {
        $ntwise_abstract_annot[ $r - $current_zero ] = 'M';
      }
    }
  ...
  # print nucleotide-wise sequence and annotation
  if ( defined $i_abstr_out ) {
    say $out_abstr @ntwise_abstract_annot;
  }
  ...
  if ($stacks) {
    ...
    for ( my $i = 1 ; $i < @pairs ; $i++ ) {
        ...
        && $curr_bp_aref->[1] == $prev_bp_aref->[1] + 1 )
      {
        ...
        # add four edges from involved nucleotides
        # the new stacking node is in the middle of two stacked base pairs
        # and the egde arrow goes out from the middle node to each of the
        # four bases involved in the stack (the direction of edges is
        # important so that the graph remains acyclic)
        push( @stacking_info, "e $curr_gi $prev_bp_aref->[0] p" );
        push( @stacking_info, "e $curr_gi $prev_bp_aref->[1] p" );
        push( @stacking_info, "e $curr_gi $curr_bp_aref->[0] p" );
        push( @stacking_info, "e $curr_gi $curr_bp_aref->[1] p" );
